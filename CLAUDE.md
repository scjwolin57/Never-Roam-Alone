...
### 5.1 Inventory of city.html (as of 2026-09-25, "From Roamers" group of approved suggestions on each stay / eat / coffee / drink card)
...
| 04 Quarters | **where to go out**: one pick per kind (dive / party / cocktail / pub, or the alcohol-free cafe / hangout / mocktail) per hood, name + one-line note + Google place ID (map link opens that exact venue); a pick with `near` is a **neighbor-hood pick** (a spare qualifying venue of another hood within 1.5 km, shown with a rust "Nearby: <hood>" flag, map link to that hood; decisions.md 2026-09-22); `near` equal to the pick's own hood index is a **just-outside pick** (within 300 m outside the hood's boundary, shown with a "Just outside <hood>" flag; search area per hood in `_guidebuild/hoodpicks/areas/`: whole boundary, 800 m, or 1.2 km; decisions.md 2026-09-24) | `bars` (`k`,`n`,`d`,`pid`,`near`) | 40/893 have it (baseline, no place ID); the 412 cities with 500,000+ international visitors are being filled in batches of 25 by `hoodpicks/` (RESUME_hood-picks.md); new cities: run the same pipeline |
...
| 04 Quarters | **From Roamers**: under our picks in each stay / eat / coffee / drink card, a dashed line and a rust "From Roamers · checked by us" head with a Show / Hide text button (one choice for the page, kept in the browser), then the approved visitor suggestions for that card as ordinary pick rows; a pick a signed-in member sent ends with their profile picture and name (from `public_profiles`) linked to their profile. Shown only when the card has one; stay picks use the lodging tier as their label (decisions.md 2026-09-25, option C) | `roamer_picks` (`{eat,cafes,bars,stay}` → 5 lists, pick shape plus `by`) | `hoodpicks/load_picks.py roamer` (one Origin = roamer row in the Hood Picks tab, after the hood-picks checks) then `apply`; a new city normally has none |
| 04 Quarters | **Suggest a place**: a button at the bottom of each stay / eat / coffee / drink card opens a form (the card's listing, required; Google Maps link, required; website, notes, "contact me when published", optional; email only for a visitor who is not signed in) | user-generated: `submit-suggestion.js` → Supabase `place_suggestions` (pending, admin-only, no public view) → Admin → Suggestions; "Mark published" (`approve-suggestion.js`) only after the venue has passed the hood-picks checks and is in the sheet, then thanks the person (inbox message for a member, else email) and gives a member the recommendation points (decisions.md 2026-09-24) | nothing to research; site-only, not in citydata or the sheet |
...
| Sheet | one row, 254 columns, incl. Intl Visitors basis/note, Beaches Score, the laundry columns, Hood N Latitude/Longitude, Landmark N Photo File/Credit and Food N Name/Photo File/Credit (added 2026-09-19); plus the Laundry, Gyms, Laundromats and Hood Picks venue tabs (Hood Picks: one row per eat / cafes / bars pick with place ID, note, source and article URLs, plus Origin and Roamer ID since 2026-09-25 for the approved visitor suggestions of all four sections; the source of truth for `eat`, `cafes`, `bars` and `roamer_picks`) | `NRA-MASTER.xlsx` Live Cities, Laundry, Gyms, Laundromats, Hood Picks | add_city_to_sheet.py; `gyms/load_gyms.py` for the Gyms tab; `hoodpicks/load_picks.py` for the Hood Picks tab and the three citydata keys |
