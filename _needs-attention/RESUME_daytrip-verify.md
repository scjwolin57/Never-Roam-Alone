...
9. **Flights are not an option as the way to get there** (amended 2026-09-21: a flight that is part of the activity, like a scenic flight over the Okavango, is fine). A place that can only be reached as a day trip by flying is removed (18 entries mention a flight today: Abu Simbel from Aswan, Kaieteur Falls from Georgetown, Mount Yasur from Port Vila ...). If a road or boat route inside the time limits exists, keep it and rewrite the blurb without the flight.
...
- **Step 5: DONE 2026-09-21.** Regenerated `NRA-MASTER.xlsx`, sheet "Live Cities", columns EK "Half-Day Trips" and EL "Full-Day Trips" from `day-trips.js` for all 893 cities. Cell format: newline-separated entries, each `"<Name> (<Country>) — <blurb>"`, `★` prefix on entries with `featured: true`. Verified 0/893 mismatches after the sync, and diffed every other cell across all 9 sheets (About, Gaps, Pending Day Trips, Live Cities, Second Tier Cities, Bucket List, Events, Laundry, Gyms) against the pre-sync version to confirm nothing else moved.
  **Gotcha hit and fixed:** `openpyxl`'s `Worksheet.cell(row, col, value=None)` treats `None` as "don't set anything" (it's the method's default-argument sentinel), not "clear this cell." A first pass therefore added/updated every cell that should now hold text, but silently skipped clearing any cell that should now be empty (day trips fully removed) -- 133 of 893 cities were mismatched after that pass. Fixed by getting the cell object first and assigning `.value` directly, which does clear correctly. Anyone writing to this sheet programmatically should watch for the same trap.
...
Note: 5 more cities (Ahmedabad, Koror, Ulaanbaatar, Weno, Yaren) exist in `citydata/_index.json` (893 total) but were never in this pass's batch plan -- see the rule-12 backlog note above. All 893 cities now have a `day-trips.js` entry (Ahmedabad's was missing, added 2026-09-21), but only 888 were actually verified by this pass.
