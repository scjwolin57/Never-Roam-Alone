...
- **Blocked on Jeff:** `hood-picks-batch1-load.md` (hood-fit-definition.md is resolved: reading A) (which reading of "in the hood"). Then: one more fresh 10% audit under that reading → `load_picks.py stage work/batch01r/final` → `apply` → `parity` → browser check of a `near` flag → commit.
