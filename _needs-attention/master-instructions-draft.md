# Master instructions for working with Jeff — DRAFT for review

*Drafted 2026-09-16 from an audit of 92 past sessions (36 Claude Code, 56 Cowork),
the commit history, CLAUDE.md, the memory notes and the needs-attention folder.
Once approved, this becomes the project CLAUDE.md (the destructive-edit and git
rules already there are kept in full below) and a copy goes to ~/.claude/CLAUDE.md
so it applies to every project, in Code and in Cowork.*
...
| Sheet | one row, 204 columns, incl. Intl Visitors basis/note, Beaches Score, the laundry columns (now shown on the hotel card), Hood N Latitude/Longitude | `NRA-MASTER.xlsx` Live Cities | add_city_to_sheet.py |
...
## 7. Files, edits and safety (unchanged from the current CLAUDE.md)
